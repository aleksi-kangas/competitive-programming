...
int QueryRangeMinimum(int node_idx, Range node, Range query) {
  ...
  return min(
      QueryRangeMinimum(node_idx * 2, {node.low, mid}, query),
      QueryRangeMinimum(node_idx * 2 + 1, {mid + 1, node.high}, query)
  );
}

void UpdateMinimumTree(int idx, int value) {
...
void PerformQuery() {
  ...
  switch (type) {
    ...
    case 2:  // Query
    {
      ...
      --a;
      --b;
      ll answer = QueryRangeMinimum(1, {0, static_cast<int>(segment_tree.size() / 2) - 1}, {a, b});
      cout << answer << '\n';
      break;
    }
    default:
...
void Solve() {
  ...
  segment_tree.resize(2 * m, numeric_limits<int>::max());
