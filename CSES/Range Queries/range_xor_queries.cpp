...
int QueryRangeXorSum(int node_idx, Range node, Range query) {
  ...
  return QueryRangeXorSum(2 * node_idx, {node.low, mid}, query)
      ^ QueryRangeXorSum(2 * node_idx + 1, {mid + 1, node.high}, query);
}
...
void Solve() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    --low;
    --high;
    int answer = QueryRangeXorSum(1, {0, m - 1}, {low, high});
    cout << answer << '\n';
  }
}
